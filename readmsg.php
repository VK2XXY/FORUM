...
$q=mysqli_query($conn, "select *,date_format(times, '%d/%m/%Y %H:%i') as ttimes from $mysql_table where id=$id");
...
<!--
<?php
if(!(preg_match("/search\.php/", $http_referer)))
{
...
<tr><td width="2%" class=t>&nbsp;</td><td class="t"><?php if ($html_enabled) echo "<pre>"; ?><?php echo ($content!="")?$content:"&nbsp"; ?><?php if ($html_enabled) echo "</pre>" ?></td><td width="2%" class=t>&nbsp;</td></tr>
</table>
</td></tr>
...
<td align="center">
<A href="newmsg.php?id=<?php echo $id;?>&sub_thread=1&days=<?php echo $days; ?>&js=<?php echo $js; ?>&lang=<?php echo $lang; ?>" onClick="window.status=''; this.href=MakeMsg('id=<?php echo $id;?>&sub_thread=1&js=<?php echo $js; ?>&lang=<?php echo $lang; ?>')"<?php mouse_text($msg["open_subthread"]);?>><?php echo $msg["open_subthread"]?></A>
</td>
